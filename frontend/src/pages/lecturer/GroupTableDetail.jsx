...
import {
  ...
  Sparkles, Check, UploadCloud, Trash2, DownloadCloud
} from 'lucide-react';
...
//handle Auto Assign Ai Part
  ...
  const handleManualAssign = async (studentId, groupId) => {
    ...
    try {
      ...
      await API.post(`/groups/${groupId}/assign`, { studentId, joinMethod: 'manual' });
  ...
  const handlePublish = async () => {
    const msg = unassigned.length > 0
      ? `There are ${unassigned.length} unassigned students. Do you still want to publish?`
      : 'Are you ready to publish these groups to students?';
  ...
  const handleDeleteAssignment = async () => {
    const ok = window.confirm(`Delete "${project.assignmentTitle}"? All groups and registrations will be removed permanently.`);
    if (!ok) return;
    try {
  ...
  const handleExportExcel = () => exportGroupsToExcel(project, groups);
  ...
  const renderAvatar = (name, index) => {
    ...
    return (
      <div key={index} className={`w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-xs font-bold -ml-2 first:ml-0 shadow-sm ${colors[index % colors.length]}`} title={name}>
      ...
      <div className="bg-[#f8f9fc] min-h-[calc(100vh-64px)] flex flex-col relative pb-24">
        ...
        <div className="px-8 mt-6">
          <div className="flex items-center gap-2 text-sm text-gray-500 font-medium mb-6">
            <span className="cursor-pointer hover:text-gray-900" onClick={() => navigate('/lecturer/groups')}>Groups</span>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 mt-4">
            <div className="max-w-xl">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <button type="button" onClick={handleDeleteAssignment} disabled={deletingAssignment}
                ...
                <button type="button" onClick={handleExportExcel}
                  className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors">
                  <DownloadCloud className="w-4 h-4" /> Download as Excel
                </button>
              </div>
            </div>
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-100 flex gap-3 bg-white">
              <div className="flex-1 relative">
                ...
                <input type="text" placeholder="Search groups or members..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredGroups.map(group => {
                ...
                if (isFull) { statusBadge = <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-green-600 border border-green-200 uppercase tracking-wide">Full</span>; barColor = 'bg-green-500'; }
                else if (!isEmpty) { statusBadge = <span className="px-3 py-1 rounded-full text-xs font-bold bg-orange-50 text-orange-600 border border-orange-200 uppercase tracking-wide">Partial</span>; barColor = 'bg-orange-500'; }
                ...
                return (
                  <div key={group._id} className="grid grid-cols-12 gap-4 px-6 py-5 border-b border-gray-50 items-center hover:bg-gray-50/50 transition-colors group/row">
                    <div className="col-span-3">
                      ...
                      <p className="text-xs text-gray-400 mt-1 font-medium">Deadline: {formatDate(project.registrationDeadline)}</p>
                    </div>
                    <div className="col-span-3 flex items-center">
                      <div className="flex items-center">
                        {group.members.map((m, i) => renderAvatar(m.user?.fullName || m.name || 'S', i))}
                        {Array.from({ length: emptySlots }).map((_, i) => (
                          <div key={`empty-${i}`} className="w-8 h-8 rounded-full border border-dashed border-gray-300 flex items-center justify-center -ml-2 first:ml-0 bg-gray-50 text-gray-300 font-bold">+</div>
                        ))}
                      </div>
                    </div>
                    <div className="col-span-3 flex items-center gap-3">
                      <span className={`font-bold text-sm ${isFull ? 'text-green-600' : isEmpty ? 'text-gray-400' : 'text-orange-500'}`}>{filledCount}/{project.groupSize}</span>
                      <div className="flex-1 max-w-[80px] h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${barColor}`} style={{ width: `${capacityRatio * 100}%` }} />
                      </div>
                    </div>
                    <div className="col-span-2 flex items-center">{statusBadge}</div>
              ...
              {filteredGroups.length === 0 && (
                <div className="text-center py-12"><p className="text-gray-500 font-medium">No groups found matching your search.</p></div>
              )}
            </div>
          </div>
          ...
          <div className="w-full lg:w-80 xl:w-96 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col max-h-[500px]">
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="px-2.5 py-1 bg-red-50 text-red-600 text-xs font-bold rounded-lg border border-red-100 uppercase tracking-wide">{unassigned.length} Left</span>
              </div>
              <p className="text-sm text-gray-500 mb-5 leading-relaxed">Click a student to manually place them in a specific group slot.</p>
                    ...
                    <div key={student._id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl hover:shadow-sm transition-shadow bg-gray-50/30 group">
                      <div className="flex items-center gap-3 min-w-0 pr-2">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500 truncate mt-0.5 font-medium">{student.studentId}</p>
                        </div>
                      </div>
                      {assigningStudentId === student._id ? (
                        <select className="text-xs border border-blue-200 bg-blue-50 text-blue-700 rounded-lg p-1.5 focus:outline-none" autoFocus
                          onBlur={() => setAssigningStudentId(null)} onChange={(e) => handleManualAssign(student._id, e.target.value)}>
                        ...
                        <button onClick={() => setAssigningStudentId(student._id)}
            ...
            <div className="bg-[#eff4fb] rounded-2xl border border-blue-100 p-5 relative overflow-hidden">
              ...
              <p className="text-sm text-blue-800/80 mb-4 leading-relaxed relative z-10">The algorithm will instantly prioritize balanced allocations across all open slots.</p>
              ...
              <div className="absolute right-0 bottom-0 w-32 h-32 bg-blue-500/5 rounded-tl-full -mr-8 -mb-8 z-0" />
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.02)] z-30 lg:pl-[280px]">
          <div className="max-w-7xl mx-auto px-8 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              {!isPublished && (
                <>
                  <button onClick={handleAutoAssign} disabled={assigning || unassigned.length === 0}
                  ...
                  <button onClick={handlePublish} disabled={publishing}
                    ...
                    <UploadCloud className="w-5 h-5" /> Confirm & Publish Final Groups
                  </button>
                </>
              )}
              {isPublished && (
                <button onClick={handleExportExcel}
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl transition-colors shadow-sm">
                  <DownloadCloud className="w-5 h-5" /> Export Groups as Excel
                </button>
              )}
            </div>
          </div>
