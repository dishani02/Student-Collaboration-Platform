...
const LecturerProfile = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) { setCatalogModules([]); toast.error('Could not load the module list.'); }
      } finally {
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        bio: user.bio || '',
        teachingModules: (user.teachingModules || []).map((c) => String(c).trim().toUpperCase()).filter(Boolean)
      });
    }
  }, [user]);
  ...
  const handleModuleSelection = (moduleCode) => {
    ...
    setFormData((prev) => {
      ...
      if (current.includes(upper)) return { ...prev, teachingModules: current.filter((c) => c !== upper) };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!userId) { toast.error('Could not determine your account id. Please log in again.'); return; }
    setLoading(true);
    try {
      const payload = {
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-primary-100 rounded-full flex items-center justify-center border-4 border-white shadow-md">
              <span className="text-2xl font-bold text-primary-700">{user.fullName?.charAt(0)}</span>
            </div>
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Lecturer Profile</h1>
              <p className="text-gray-600 mt-1">Manage your public information and teaching context.</p>
            </div>
          </div>
          {!isEditing ? (
        ...
        <Card className="bg-white">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input label="Full Name" name="fullName" icon={User} value={formData.fullName} onChange={handleChange} disabled={!isEditing} required />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input label="Lecturer ID" name="lecturerId" icon={Briefcase} value={formData.lecturerId} disabled helperText="ID is issued by administration." />
              <Input label="Department" name="department" icon={Building2} value={formData.department} disabled helperText="Department cannot be changed." />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {isEditing ? (
                <Select label="Position" name="position" value={formData.position} onChange={handleChange}
                  options={[{ value: 'Lecturer', label: 'Lecturer' }, { value: 'Senior Lecturer', label: 'Senior Lecturer' }, { value: 'Professor', label: 'Professor' }]}
                  required />
              ) : (
                ...
                <div className="border border-primary-200 rounded-lg p-4 bg-primary-50/30 max-h-[28rem] overflow-y-auto space-y-2">
                  ...
                  {catalogModules.map((module) => (
                    <label key={module.code} className="flex items-start gap-3 p-3 rounded-lg hover:bg-white cursor-pointer transition-colors border border-transparent hover:border-primary-100">
                      <input type="checkbox" checked={formData.teachingModules.includes(module.code)} onChange={() => handleModuleSelection(module.code)}
