...
const Groups = () => {
  ...
  const filteredProjects = tabSource.filter((p) => {
    const matchesSearch =
      p.assignmentTitle.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleDeleteProject = async (e, project) => {
    ...
    const ok = window.confirm(
      `Delete "${project.assignmentTitle}" (${project.module})? All groups and member slots for this assignment will be removed. This cannot be undone.`
    );
    if (!ok) return;
    try {
    ...
    <div className="text-center py-20 bg-white border border-dashed border-gray-300 rounded-xl">
      ...
      <p className="text-gray-500 max-w-md mx-auto mb-6">
        {search || selectedModule !== 'All'
          ...
          : tab === 'published'
            ? 'Nothing has been published yet. Use Confirm & Publish on an assignment when you are ready.'
            : "You don't have any open assignments. Create a new group table to get started."}
      </p>
      {search || selectedModule !== 'All' ? (
        <Button
          variant="outline"
        ...
        <div className="flex items-center justify-between mb-6 flex-shrink-0">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage registration tables and final published rosters separately.
            </p>
          </div>
          <Button icon={PlusCircle} onClick={() => navigate('/lecturer/groups/create')}>
                  ...
                  >
                    <div className="flex justify-between items-start gap-2 mb-4">
                      <div className="min-w-0">
                        <Badge variant="default" className="mb-2 uppercase text-xs tracking-wider font-semibold">
                          {project.module}
                        </Badge>
                      ...
                      {project.status === 'published' && (
                        <button
                          ...
                          onClick={(e) => handleExportQuick(e, project)}
                          className="flex-shrink-0 p-2 rounded-lg border border-secondary-200 text-secondary-600 hover:bg-secondary-50 ml-2"
                        >
