...
import { 
  Users, CalendarClock, Clock, PlusCircle, BookOpen, AlertCircle, FileText
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const upcoming = [...active].sort((a, b) => new Date(a.registrationDeadline) - new Date(b.registrationDeadline)).slice(0, 3);
      ...
      for (const p of active) {
        try {
          ...
          if (unassignedCount > 0) stats.push({ project: p, count: unassignedCount });
        } catch (e) {
          console.error(`Error fetching unassigned for ${p._id}`);
        }
      }

      setDashboardData({ activeProjects: active, unregisteredStats: stats, upcomingDeadlines: upcoming });
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="p-8">
        <div className="mb-8 flex justify-between items-end border-b border-gray-200 pb-6">
          ...
          <Button icon={PlusCircle} onClick={() => navigate('/lecturer/groups')}>Create Group Table</Button>
        </div>
          ...
          <Card className="bg-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-4xl font-bold text-gray-900">{user?.teachingModules?.length || 0}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <div className="flex justify-between items-center mb-4">
                ...
                <Button variant="ghost" size="sm" onClick={() => navigate('/lecturer/groups')}>Manage All →</Button>
              </div>
              {dashboardData.activeProjects.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <Button size="sm" variant="outline" onClick={() => navigate('/lecturer/groups')}>Create Assignment</Button>
                </div>
              ) : (
                <div className="space-y-4">
                  {dashboardData.activeProjects.slice(0, 5).map((project) => {
                    ...
                    return (
                      <div key={project._id} className="bg-white border rounded-lg p-5 hover:border-primary-300 transition-colors shadow-sm">
          ...
          <div className="space-y-6">
                  ...
                  {dashboardData.unregisteredStats.map((stat) => (
                    <div key={stat.project._id} className="p-3 border rounded-lg bg-orange-50/50 border-orange-100">
                      ...
                      <div className="flex justify-between items-center">
                        <p className="text-xs text-gray-600">{stat.project.module}</p>
                        <Badge variant="warning">{stat.count} missing</Badge>
                      </div>
                      <Button variant="outline" size="sm" className="w-full mt-3 text-xs py-1" onClick={() => navigate('/lecturer/groups')}>Auto-Assign</Button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <Card>
              <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                <Clock className="w-5 h-5 text-primary-500" /> Deadlines
              </h2>
                  ...
                  {dashboardData.upcomingDeadlines.map((p) => {
                    ...
                    return (
                      <div key={p._id} className="flex justify-between items-center border-b pb-2 last:border-0 last:pb-0">
                        ...
                        <span className={`text-xs font-bold px-2 py-1 rounded ${days < 0 ? 'bg-red-100 text-red-700' : days <= 3 ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}>
