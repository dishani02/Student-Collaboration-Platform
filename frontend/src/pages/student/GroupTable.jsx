...
const StudentGroupTable = () => {
  ...
  useEffect(() => {
    if (user) {
      setIndiv(prev => {
        const next = { ...prev };
        if (!next.fullName && user.fullName) next.fullName = user.fullName;
        if (!next.studentId && user.studentId) next.studentId = user.studentId;
  ...
  const fetchData = async () => {
    try {
      ...
      setGroupTable(projectRes.data.groupTable);
    ...
    setFullMembers((prev) => {
      ...
      if (prev.length === n) {
        if (user) {
          const next = [...prev];
          const first = { ...next[0] };
          if (!first.fullName && user.fullName) first.fullName = user.fullName;
          if (!first.studentId && user.studentId) first.studentId = user.studentId;
          if (!first.email && user.email) first.email = user.email;
          if (!first.phone && user.phone) first.phone = user.phone;
          next[0] = first;
          return next;
        }
        return prev;
      ...
      return Array.from({ length: n }, (_, i) => {
        if (i === 0 && user) {
          return { fullName: user.fullName || '', studentId: user.studentId || '', email: user.email || '', phone: user.phone || '', isLeader: false };
        }
  ...
  const handleFullRowChange = (index, field, value) => {
    setFullMembers((prev) => { const next = [...prev]; next[index] = { ...next[index], [field]: value }; return next; });
  };
  ...
  const handleStudentIdBlur = async (index, studentId) => {
    if (!studentId || !studentId.trim()) return;
    try {
      const res = await API.get(`/users/lookup/${studentId.trim()}`);
      if (res.data.success && res.data.user) {
        setFullMembers((prev) => {
          const next = [...prev];
          next[index] = { 
            ...next[index], 
            fullName: res.data.user.fullName || next[index].fullName, 
            email: res.data.user.email || next[index].email,
            phone: res.data.user.phone || next[index].phone
          };
          return next;
        });
      }
  ...
  const submitFullGroup = async (e) => {
    e.preventDefault();
    if (!selectedEmptyGroup) { toast.error('Choose an empty group.'); return; }
    const g = groups.find((x) => String(x._id) === String(selectedEmptyGroup));
    if (!g) return;
    const filled = fullMembers.filter((m) => m.fullName?.trim() && m.studentId?.trim());
    
    // Check for duplicate student IDs
    const studentIds = filled.map(m => m.studentId.trim().toLowerCase());
    const uniqueIds = new Set(studentIds);
    if (uniqueIds.size !== studentIds.length) {
      toast.error('The same Student ID cannot be added twice.');
      return;
    }

    if (filled.length !== g.maxMembers) { toast.error(`Enter all ${g.maxMembers} members.`); return; }
    if (!filled.some((m) => m.isLeader)) { toast.error('Mark one member as group leader.'); return; }
    try {
      setSubmittingFull(true);
      await API.post(`/groups/project/${projectId}/register-full-group`, {
        groupNumber: g.groupNumber,
        members: filled.map((m) => ({ fullName: m.fullName.trim(), studentId: m.studentId.trim(), email: m.email?.trim(), phone: m.phone?.trim(), isLeader: !!m.isLeader }))
      });
  ...
  const handleIndivStudentIdBlur = async (studentId) => {
    if (!studentId || !studentId.trim()) return;
    try {
      const res = await API.get(`/users/lookup/${studentId.trim()}`);
      if (res.data.success && res.data.user) {
  ...
  const submitIndividual = async (e) => {
    ...
    try {
      ...
      await API.post(`/groups/project/${projectId}/register-individual`, {
        fullName: indiv.fullName.trim(), studentId: indiv.studentId.trim(), email: indiv.email.trim(), phone: indiv.phone.trim()
      });
  ...
  return (
    <DashboardLayout>
      <div className="bg-[#f8f9fc] min-h-[calc(100vh-64px)] flex flex-col relative pb-12">
        {registrationLocked && (
          <div className={`mx-8 mt-6 rounded-xl border px-4 py-3 flex items-center gap-3 ${isPublished ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-amber-50 border-amber-200 text-amber-900'}`}>
            <Lock className="w-5 h-5 flex-shrink-0" />
            <div>
              <p className="font-bold text-sm">{isPublished ? 'Final groups published — view only' : 'Registration closed'}</p>
              <p className="text-xs opacity-90 mt-0.5">
                {isPublished ? 'Your lecturer has published this table. Registration and edits are no longer available.' : 'The registration deadline has passed. You can still view the table below.'}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="px-8 mt-6">
          <div className="flex items-center gap-2 text-sm text-gray-500 font-medium mb-4">
            <button type="button" onClick={() => navigate('/student/groups')} className="inline-flex items-center gap-1 text-gray-600 hover:text-gray-900">
              <ArrowLeft className="w-4 h-4" /> Groups
            </button>
            <ChevronRight className="w-4 h-4" />
            <span className="text-gray-900">{groupTable.module}: {groupTable.assignmentTitle}</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 mt-2">
            <div className="max-w-xl">
              ...
              <p className="text-gray-500 text-lg leading-relaxed">
                ...
                Register with your group or take the next free slot.
              </p>
            ...
            <div className="flex items-center gap-4 bg-white px-5 py-3 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="w-32 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full transition-all duration-700" style={{ width: `${fillPercentage}%` }} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-100 flex gap-3 bg-white">
              <div className="flex-1 relative">
                ...
                <input type="text" placeholder="Search groups or names..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              <button type="button" className="px-3.5 py-2.5 bg-white border border-gray-200 rounded-xl text-gray-500 hover:bg-gray-50 shadow-sm" aria-label="Filter">
            ...
            <div className="flex-1 overflow-y-auto max-h-[min(60vh,520px)]">
              {filteredGroups.map((group) => {
                ...
                return (
                  <div key={group._id} className={`grid grid-cols-12 gap-4 px-6 py-4 border-b border-gray-50 items-center ${isFull ? 'bg-green-50/80' : 'hover:bg-gray-50/50'}`}>
                    <div className="col-span-2 font-bold text-gray-900">Group {getGroupName(group.groupNumber)}</div>
                    <div className="col-span-3 text-sm text-gray-600">
                      <ul className="space-y-1">{(group.members || []).map((m, i) => <li key={i}>{m.studentId || '—'}</li>)}{(group.members || []).length === 0 && <li>—</li>}</ul>
                    </div>
                    <div className="col-span-5 text-sm text-gray-800 font-medium">
                      <ul className="space-y-1">{(group.members || []).map((m, i) => <li key={i}>{m.name || '—'}</li>)}{(group.members || []).length === 0 && <li>—</li>}</ul>
                    </div>
                    <div className="col-span-2 text-right text-sm font-semibold text-gray-700">{filled}/{cap}</div>
                  </div>
                );
              })}
              {filteredGroups.length === 0 && <div className="text-center py-12 text-gray-500 font-medium">No rows match your search.</div>}
            </div>
          </div>
          ...
          <div className="w-full lg:w-80 xl:w-96 flex flex-col gap-6">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <dl className="text-sm space-y-2 text-gray-600">
                <div className="flex justify-between gap-2"><dt className="text-gray-400">Module</dt><dd className="font-medium text-gray-900">{groupTable.module}</dd></div>
                <div className="flex justify-between gap-2"><dt className="text-gray-400">Year / Sem</dt><dd className="font-medium text-gray-900">{groupTable.yearLevel} · Sem {groupTable.semester}</dd></div>
                <div className="flex justify-between gap-2"><dt className="text-gray-400">Specialization</dt><dd className="font-medium text-gray-900">{groupTable.specialization}</dd></div>
              </dl>
            </div>
          </div>
        </div>
        ...
        {canRegister && !myGroup && (
          <div className="px-8 mt-10 grid grid-cols-1 xl:grid-cols-2 gap-8">
            <form onSubmit={submitFullGroup} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
              <h3 className="text-lg font-bold text-gray-900">Full group register</h3>
              <p className="text-sm text-gray-500">The group leader registers every member at once.</p>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Empty group</label>
                <select value={selectedEmptyGroup} onChange={(e) => setSelectedEmptyGroup(e.target.value)}
                  ...
                  {emptyGroups.map((g) => <option key={g._id} value={g._id}>Group {getGroupName(g.groupNumber)} ({g.maxMembers} members)</option>)}
                </select>
                {emptyGroups.length === 0 && <p className="text-xs text-amber-600 mt-1">No completely empty groups left.</p>}
              </div>
              {selectedEmptyGroup && fullMembers.map((row, idx) => (
                <div key={idx} className="border border-gray-100 rounded-xl p-4 space-y-2 bg-gray-50/40">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <Input label="Full name" value={row.fullName} onChange={(e) => handleFullRowChange(idx, 'fullName', e.target.value)} required />
                    <Input label="Student ID" value={row.studentId} onChange={(e) => handleFullRowChange(idx, 'studentId', e.target.value)} onBlur={() => handleStudentIdBlur(idx, row.studentId)} required />
                    <Input label="Email" type="email" value={row.email} onChange={(e) => handleFullRowChange(idx, 'email', e.target.value)} />
                    <Input label="Phone" value={row.phone} onChange={(e) => handleFullRowChange(idx, 'phone', e.target.value)} />
                  </div>
                  <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                    <input type="radio" name="leader" checked={row.isLeader} onChange={() => handleLeaderRadio(idx)} /> Group leader
                  </label>
                </div>
              ))}
            ...
            <form onSubmit={submitIndividual} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
              <h3 className="text-lg font-bold text-gray-900">Individual slot</h3>
              <p className="text-sm text-gray-500">Register only yourself. You will be placed in the next available slot.</p>
              <Input label="Full name" value={indiv.fullName} onChange={(e) => setIndiv((p) => ({ ...p, fullName: e.target.value }))} required />
              <Input label="Student ID" value={indiv.studentId} onChange={(e) => setIndiv((p) => ({ ...p, studentId: e.target.value }))} onBlur={() => handleIndivStudentIdBlur(indiv.studentId)} required />
              <Input label="Email" type="email" value={indiv.email} onChange={(e) => setIndiv((p) => ({ ...p, email: e.target.value }))} required />
              <Input label="Phone" value={indiv.phone} onChange={(e) => setIndiv((p) => ({ ...p, phone: e.target.value }))} />
